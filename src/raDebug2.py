...
if __name__ == '__main__':
    ...
    #target 1    
    initial_time=5140000 #in frame number
    final_time = 6150000 #in frame number
    ...
    for mceFN in ds.df.index:
        ...
        qI2Starcam_list.append(qStarcam2Gyros_old.inv()*q_sc) #no other way to get the SC solution :(
    triggers=ds.df['triggers'].drop_duplicates()
    i2s=pd.DataFrame({'qI2S': qI2Starcam_list},index=ds.df.index)
    ...
    difs=[[],[],[]]
    for i in range(len(triggers.values)-1):
        t_start=triggers.iloc[i]
        t_end=triggers.iloc[i+1]
        gyros=meas.loc[t_start:t_end].dropna(subset=['gyroX'])
        M=np.eye(4)
        for j in range(len(gyros.index)):
            dt=(meas.index[j+1]-meas.index[j])/400.
            ...
            M=scipy.linalg.expm(0.5*Ow*dt).dot(M)
        for j in range(len(qStarcam2Gyros)):
            q_old=qStarcam2Gyros[j]*meas['qI2S'].loc[t_start]
            q_new=qStarcam2Gyros[j]*meas['qI2S'].loc[t_end]
            q_prop=Quat(M.dot(q_old.q))
            q_dif=q_prop*q_new.inv()
            difs[j].append(q_dif)
    ...
    fig=plt.figure()
    ax=plt.subplot(111)
    plt.plot(triggers.index[:-1],[q.ra for q in difs[0]])
    plt.plot(triggers.index[:-1],[q.ra for q in difs[1]])
    plt.plot(triggers.index[:-1],[q.ra for q in difs[2]])
    ax.legend(['old','mid','new'])
    ax.set_xlabel('Time (frames)')
    ax.set_ylabel('RA (deg)')
        
    fig=plt.figure()
    ax=plt.subplot(111)
    plt.plot(triggers.index[:-1],[q.dec for q in difs[0]])
    plt.plot(triggers.index[:-1],[q.dec for q in difs[1]])
    plt.plot(triggers.index[:-1],[q.dec for q in difs[2]])
    ax.legend(['old','mid','new'])
    ax.set_xlabel('Time (frames)')
    ...
    fig=plt.figure()
    ax=plt.subplot(111)
    plt.plot(triggers.index[:-1],[q.roll for q in difs[0]])
    plt.plot(triggers.index[:-1],[q.roll for q in difs[1]])
    plt.plot(triggers.index[:-1],[q.roll for q in difs[2]])
    ax.legend(['old','mid','new'])
    ax.set_xlabel('Time (frames)')
