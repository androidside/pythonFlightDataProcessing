...
class Estimator(object):
    ...
    def plot(self,save_folder=None,time_label='Time (frame number)'):
        ...
        df=pd.merge(self.est,self.sc,how='outer',left_index=True,right_index=True)
        ...
        if save_folder is not None: f.savefig(save_folder+"estimator.png")
...
class Estimator3(Estimator):
    ...
    def estimate(self,P0=0*np.eye(3),q0=None,Qd=np.eye(3),SCg=5,ts=None,te=None,progress=False):
        ...
        for j in range(L-1):
            ...
            w=(gyros.iloc[j,:3].as_matrix())*(1/3600.*np.pi/180) #arcsec2rad conversion -bias
            ...
            if nextSCindex < len(sc.index) and ind >= sc.index[nextSCindex]: #update!
                ...
                R=(SCg*np.diag(sc.iloc[nextSCindex][['ra_err','dec_err','roll_err']].tolist()))**2
                R=Mrot*R*Mrot.T
