...
class DataSet():
    def __init__(self,folder,freq=100,min=None,max=None,folder_export = None,gyros=False,estimator=False,starcam=False,droplist = []):
        '''
        ...
        self.timeRTHighPriority = load_single_field(folder+'bettii.RTHighPriority.mceFrameNumber','i8')
        self.timeRTLowPriority = load_single_field(folder+'bettii.RTLowPriority.mceFrameNumber','i8')
        
        self.df = pd.DataFrame({'timeRTHighPriority':self.timeRTHighPriority},index=self.timeRTHighPriority)
        ...
        if min == None and max==None:
            ...
            self.df = self.df.loc[:max*400,:]
        elif max == None and min!=None:
            pass#self.df = self.df.loc[min*400:,:]
        else:
            self.df = self.df.loc[min*400:max*400,:]
        self.freq = freq
    ...
    def readField(self,fieldName,timeName=None,field_type='f8',time_type='i8',folder=None,timeDivider=1):
        if folder is None: folder=self.folder
        
        if timeName is None: time=self.timeHighPriority
        else: time = load_single_field(folder+timeName,time_type)
        
        field = load_single_field(folder+fieldName,field_type)

        df_tmp = pd.DataFrame({'fieldName':field},index=time/timeDivider)
        df_tmp.drop_duplicates(inplace=True)
        self.df = pd.merge(self.df,df_tmp,how='inner',left_index=True,right_index=True) 
        
    def readRaDec(self,folder=None,timeDivider=1):
        if folder is None: folder=self.folder
        ...
        RADEC = {'telRA':telRA, 'telDec':telDec}
        df_tmp = pd.DataFrame(RADEC,index=self.timeRTHighPriority/timeDivider)
        df_tmp.drop_duplicates(inplace=True)
        print len(df_tmp)
        df_tmp = pd.merge(self.df,df_tmp,how='inner',left_index=True,right_index=True)
        print len(df_tmp)
        self.df=df_tmp
        
    def readElevations(self,folder=None,timeDivider=1):
        if folder is None: folder=self.folder                
        ...
        df_tmp = pd.DataFrame(criticals,index=self.timeRTHighPriority/timeDivider)
        df_tmp.drop_duplicates(inplace=True)
        self.df = pd.merge(self.df,df_tmp,how='inner',left_index=True,right_index=True) 
        
        
    def readAzControl(self,folder=None,timeDivider=1):
        if folder is None: folder=self.folder
        ...
        df_tmp = pd.DataFrame(stepper,index=stepperTime/timeDivider)
        df_tmp.drop_duplicates(inplace=True)
        df_tmp2 = pd.DataFrame(momDump,index=momDumpTime/timeDivider)
        df_tmp2.drop_duplicates(inplace=True)
        df_tmp3 = pd.DataFrame(CCMG,index=CCMGTime/timeDivider)
    ...
    def readGyros(self,folder=None,timeDivider=1):
        if folder is None: folder=self.folder
        ...
        self.gyros = {'gyroX':gyroX,
                'gyroY': gyroY,
                'gyroZ':gyroZ}
        self.gyros_T = {'gyroXT':gyroXT,
                'gyroYT': gyroYT,
                'gyroZT':gyroZT}
        self.gyros_F = {'gyroXF':gyroXF,
                'gyroYF': gyroYF,
                'gyroZF':gyroZF}
        df_tmp = pd.DataFrame(self.gyros,index=time/timeDivider)
        df_tmp.drop_duplicates(inplace=True)
        df_tmp = pd.merge(self.df,df_tmp,how='inner',left_index=True,right_index=True)   
        self.df = pd.merge(self.df,df_tmp,how='inner',left_index=True,right_index=True) 
    ...
    def readEstimator(self,folder=None,timeDivider=1):
        ...
        df_estimator = pd.DataFrame(estimatorData,index = self.timeRTLowPriority/timeDivider)
        df_estimator.drop_duplicates(inplace=True)

        df_bias = pd.DataFrame(biasData,index = self.timeRTHighPriority/timeDivider)
        df_bias.drop_duplicates(inplace=True)
        df_tmp = pd.merge(self.df,df_estimator,how='inner',left_index=True,right_index=True)
        self.df = pd.merge(df_tmp,df_bias,how='inner',left_index=True,right_index=True)
    def readStarcamera(self,folder=None,timeDivider=1):
        ...
        df_solution = pd.DataFrame(starcamsol,index = self.timeRTLowPriority/timeDivider)
        ...
        self.pulses = pd.DataFrame({'duration': starcam_trigger_pulses},index = self.timeRTLowPriority/timeDivider)
        ...
        #estval = df_estimator.loc[df_solution['trigger_center']] # times 4 and divided by 2
        #values = pd.DataFrame(estval.loc[:,['ra','dec','roll']],index=df_solution.index)
        #print estval.loc[:,['ra','dec','roll']],values
        self.df_starcamera = pd.merge(df_solution,self.df[['ra','dec','roll']],how='inner',left_on='trigger_center',right_index=True)
    ...
    def simplePlot(self,val,minMax = [],ylabel="",ax_key=None,save=True,show=False,realTime=True,name=None):
        ...
        if ax_key==None:
            ...
            #plt.close(fig)
        print "Done."
    def simple2DPlot(self,field1,field2,minMax = [],xlabel="",ylabel="",ax_key=None,save=False,draw=True,realTime=True,name=None,kde = False):
        print "Loading data..."
        if ax_key==None: fig,ax = plt.subplots(figsize=(5.9,4),dpi=120)
        else: ax = ax_key
